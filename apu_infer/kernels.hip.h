/*
 * kernels.hip.h — HIP compute kernels for Flash-MoE inference (AMD ROCm)
 *
 * Port of kernels.cuh for AMD GPUs via HIP.
 * Targets: MI300X (CDNA 3, gfx942, wavefront-64),
 *          RX 7900 XTX (RDNA 3, gfx1100, wavefront-32),
 *          RX 9070 XT (RDNA 4, gfx1201, wavefront-32).
 *
...
// Set at compile time via -DWARP_SIZE=64 or auto-detected from target arch
#ifndef WARP_SIZE
...
// HIP __ldg passthrough (supported on most targets, fallback if not)
#if !defined(__ldg)
template<typename T>
...
__global__ void dequant_matvec_4bit_fma_vec4(
    ...
    const uint32_t tid = warp_id * WARP_SIZE + lane;
    for (uint32_t i = tid; i < in_dim; i += (WARP_SIZE * ROWS_PER_BLOCK))
        x_shared[i] = x[i];
    __syncthreads();
...
__global__ void dequant_matvec_q4k(
    ...
    for (uint32_t bi = tid; bi < blocks_per_row; bi += (WARP_SIZE * ROWS_PER_BLOCK)) {
        float max_abs = 0.0f;
    ...
    for (uint32_t bi = lane; bi < blocks_per_row; bi += WARP_SIZE) {
...
static inline void launch_dequant_matvec_q4k(
    ...
    uint32_t out_dim, uint32_t in_dim, hipStream_t stream = 0
) {
...
// Shared memory: WARP_SIZE * sizeof(float)
...
__global__ void rms_norm(
    ...
    if (wid == 0) {
        acc = (lane < (blockDim.x + WARP_SIZE - 1) / WARP_SIZE) ? shared[lane] : 0.0f;
        acc = warp_reduce_sum(acc);
...
__global__ void attn_scores(
    ...
    acc = warp_reduce_sum(acc);
    uint32_t wid = threadIdx.x / WARP_SIZE, lane = threadIdx.x % WARP_SIZE;
    if (lane == 0) shared[wid] = acc;
    __syncthreads();
    if (wid == 0) {
        acc = (lane < (blockDim.x + WARP_SIZE - 1) / WARP_SIZE) ? shared[lane] : 0.0f;
...
__global__ void attn_softmax(
    ...
    __syncthreads();
    if (wid == 0) {
        wmax = (lane < (blockDim.x + WARP_SIZE - 1) / WARP_SIZE) ? shared[lane] : -1e30f;
    ...
    __syncthreads();
    if (wid == 0) {
        wsum = (lane < (blockDim.x + WARP_SIZE - 1) / WARP_SIZE) ? shared[lane] : 0.0f;
...
__global__ void matvec_f32(
    ...
    for (uint32_t i = lane; i < in_dim; i += WARP_SIZE)
...
static inline void launch_dequant_matvec_gguf(
    ...
    uint32_t out_dim, uint32_t in_dim, int gguf_type, hipStream_t stream = 0
) {
    switch (gguf_type) {
        case GGUF_TYPE_Q4_K:
...
static inline void launch_swiglu(const float* gate, const float* up, float* out, uint32_t dim, hipStream_t s = 0) {
    swiglu_fused<<<(dim+255)/256, 256, 0, s>>>(gate, up, out, dim);
}

static inline void launch_rms_norm_bf16(const float* x, const uint16_t* w, float* out, uint32_t dim, float eps, hipStream_t s = 0) {
...
static inline void launch_residual_add(const float* a, const float* b, float* out, uint32_t dim, hipStream_t s = 0) {
